...
type Config = { version: number; form: Definition; defaults: Answers };
...
export default function RefundForm({ orderNumber, onBack }: { orderNumber: string | null; onBack: () => void }) {
  const query = useQuery({ queryKey: ["refund-form"], queryFn: () => json<Config>("/api/refund-form"), staleTime: Infinity });
  ...
  return <FormSession config={query.data} orderNumber={orderNumber} onBack={onBack} />;
}
function FormSession({ config: initialConfig, orderNumber, onBack }: { config: Config; orderNumber: string | null; onBack: () => void }) {
  const [config] = useState(initialConfig);
  const [answers, setAnswers] = useState<Answers>({ ...config.defaults, [ORDER_FIELD]: orderNumber && orderNumber !== "Not order-related" ? orderNumber : "" });
  ...
  async function upload(block: RefundBlock, file?: File) {
    ...
    try {
      ...
      const result = await json<{id:string;name:string}>("/api/refund-form/uploads", { method: "POST", body });
  ...
  async function proceed(e: React.FormEvent) {
    ...
    try {
      const res = await json<{ticket:{id:string};endPage:string}>("/api/refund-form", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ version: config.version, clientRequestId: getId(), answers }) });
